...
# def train_sarsa(env, params):
#     """Trains an agent using the on-policy SARSA algorithm."""
...
#             # SARSA Update: Q(S,A) <- Q(S,A) + a * [ R + g * Q(S',A') - Q(S,A) ]
#             target = reward + gamma * q_table[next_state[0], next_state[1], next_action]
#             if done:
#                  target = reward 
            
#             q_table[r, c, action] += alpha * (target - q_table[r, c, action])
...
# def train_q_learning(env, params):
#     """Trains an agent using the off-policy Q-Learning algorithm."""
#     q_table = create_q_table(env)
#     stats = {'rewards': []}
...
#             # Q-Learning Update: Q(S,A) <- Q(S,A) + a * [ R + g * max_a' Q(S',a') - Q(S,A) ]
